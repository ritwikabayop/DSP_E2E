...
import {
  ...
  Typography, Tag, Button, Tooltip, Select, Input, theme as antTheme, Form,
} from 'antd';
import {
...
const NEBULA_THEME = {
  ...
  token: {
    colorPrimary:       '#b44fff',
    colorBgBase:        '#04010f',
    colorBgContainer:   'rgba(10,2,30,0.7)',
    colorBgElevated:    'rgba(18,6,44,0.9)',
    colorBorder:        'rgba(160,80,255,0.18)',
    colorText:          '#e8d8ff',
    colorTextSecondary: '#9b6dca',
    borderRadius:       12,
    borderRadiusLG:     20,
...
const MATRIX_THEME = {
  ...
  token: {
    colorPrimary:       '#00ff32',
    colorBgBase:        '#000500',
    colorBgContainer:   'rgba(0,7,0,0.75)',
    colorBgElevated:    'rgba(0,12,0,0.9)',
    colorBorder:        'rgba(0,255,50,0.18)',
    colorText:          '#88cc88',
    colorTextSecondary: '#44aa55',
    borderRadius:       12,
    borderRadiusLG:     20,
    fontFamily:         "'Inter', 'Segoe UI', -apple-system, BlinkMacSystemFont, sans-serif",
    fontSize:           13,
...
const LAVA_THEME = {
  ...
  token: {
    colorPrimary:       '#ff4500',
    colorBgBase:        '#0f0200',
    colorBgContainer:   'rgba(15,2,0,0.72)',
    colorBgElevated:    'rgba(24,5,0,0.9)',
    colorBorder:        'rgba(255,60,0,0.2)',
    colorText:          '#ffddcc',
    colorTextSecondary: '#cc8855',
    borderRadius:       12,
    borderRadiusLG:     20,
    fontFamily:         "'Inter', 'Segoe UI', -apple-system, BlinkMacSystemFont, sans-serif",
    fontSize:           13,
...
const CHROME_THEME = {
  ...
  token: {
    colorPrimary:       '#c0c0ff',
    colorBgBase:        '#0c0c10',
    colorBgContainer:   'rgba(20,20,40,0.68)',
    colorBgElevated:    'rgba(28,28,52,0.9)',
    colorBorder:        'rgba(180,180,255,0.16)',
    colorText:          '#d0d0f0',
    colorTextSecondary: '#9999cc',
    borderRadius:       12,
    borderRadiusLG:     20,
    fontFamily:         "'Inter', 'Segoe UI', -apple-system, BlinkMacSystemFont, sans-serif",
    fontSize:           13,
...
const ZEN_THEME = {
  ...
  token: {
    colorPrimary:       '#c8aa50',
    colorBgBase:        '#020614',
    colorBgContainer:   'rgba(6,12,40,0.7)',
    colorBgElevated:    'rgba(12,20,56,0.9)',
    colorBorder:        'rgba(200,170,80,0.16)',
    colorText:          '#dcd4b0',
    colorTextSecondary: '#a08844',
    borderRadius:       12,
    borderRadiusLG:     20,
    fontFamily:         "'Inter', 'Segoe UI', -apple-system, BlinkMacSystemFont, sans-serif",
    fontSize:           13,
...
const THEME_MAP = {
  ...
  'dark-nebula':  NEBULA_THEME,
  'dark-matrix':  MATRIX_THEME,
  'candy-pop':    CANDY_THEME,
  'dark-lava':    LAVA_THEME,
  'dark-chrome':  CHROME_THEME,
  'dark-zen':     ZEN_THEME,
  'light':        LIGHT_THEME,
};
...
function App() {
  ...
  // ── Theme picker definitions (static — must be above early returns so _themeModal can use them) ──
  // Generated by GitHub Copilot
  const THEME_DEFS = [
    ...
    { key: 'dark-nebula',  name: 'Cosmic Nebula',     bg: '#04010f', accent: '#b44fff', desc: 'Deep void + cosmic purple' },
    { key: 'dark-matrix',  name: 'Matrix Code',       bg: '#000500', accent: '#00ff32', desc: 'Hacker terminal + phosphor' },
    { key: 'candy-pop',    name: 'Candy Pop',         bg: '#fff5fb', accent: '#ec4899', lightBg: true, desc: 'Pastel rainbow + vivid pink' },
    { key: 'dark-lava',    name: 'Lava Lamp',         bg: '#0f0200', accent: '#ff4500', desc: 'Dark red-black + lava orange' },
    { key: 'dark-chrome',  name: 'Galactic Chrome',   bg: '#0c0c10', accent: '#c0c0ff', desc: 'Dark + silver prism' },
    { key: 'dark-zen',     name: 'Midnight Zen',      bg: '#020614', accent: '#c8aa50', desc: 'Deep navy + soft gold' },
    { key: 'light',        name: 'Ivory Gold',        bg: '#f5f3ee', accent: '#d97706', lightBg: true, desc: 'Light + warm amber' },
  ];

  // Shared ProfileDropdown element — used by ModulePicker, KT, Roles & Access, and main app
  // Generated by GitHub Copilot
  const _profileDropdown = (
    <ProfileDropdown
      ...
      ROLES={ROLES}
      onRoleSwitch={(r) => { setViewAsRole(viewAsRole === r ? null : r); }}
      onTheme={() => setThemeModalOpen(true)}
      onSignOut={() => { signOut(); setActiveModule(null); }}
    />
  );

  // Shared Theme Picker Modal — rendered in every view so Theme Settings works everywhere
  // Generated by GitHub Copilot
  const _themeModal = (
    <Modal
      open={themeModalOpen}
    ...
    >
      <div
        ...
        style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, padding: '4px 0 8px' }}
      >
        {THEME_DEFS.map(({ key, name, bg, accent, desc, lightBg }) => {
          ...
          return React.createElement('button', {
            ...
            React.createElement('div', { style: { width: 28, height: 6, borderRadius: 4, background: accent, marginBottom: 8, boxShadow: `0 0 8px ${accent}88` } }),
            React.createElement('div', { style: { fontSize: 12, fontWeight: 700, color: lightBg ? '#1a1209' : '#ffffff', marginBottom: 2 } }, name),
            React.createElement('div', { style: { fontSize: 10, color: lightBg ? '#78530a' : 'rgba(255,255,255,0.45)' } }, desc),
            isActive && React.createElement('div', {
              'aria-hidden': 'true',
              style: { position: 'absolute', top: 8, right: 8, width: 18, height: 18, borderRadius: '50%', background: accent, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, color: '#fff', fontWeight: 900, lineHeight: 1 },
            }, '✓')
          );
        })}
      </div>
      ...
      <ConfigProvider theme={THEME_MAP[currentTheme] ?? DARK_THEME}>
        <>
          <ModulePicker
            ...
            onSignOut={() => { signOut(); setActiveModule(null); }}
            profileDropdown={_profileDropdown}
          />
          {_themeModal}
        </>
      </ConfigProvider>
  ...
  return (
    <ConfigProvider theme={THEME_MAP[currentTheme] ?? DARK_THEME}>
      ...
      {/* ── Theme Picker Modal (shared — also rendered in ModulePicker/KT/Roles via _themeModal) ── */}
      {_themeModal}
    </ConfigProvider>
  );
}
