...
-- ──────────────────────────────────────────────────────────
-- 9b. PROD_ROLES
--     Production team roles: Team Lead / Module / Resource EID / Role Details.
--     Visible and editable by admin and tl only.
-- ──────────────────────────────────────────────────────────
-- Generated by GitHub Copilot
CREATE TABLE IF NOT EXISTS prod_roles (
  ...
  module          TEXT        NOT NULL DEFAULT '',
  resource_eid    TEXT        NOT NULL DEFAULT '',
  role_details    TEXT        NOT NULL DEFAULT '',
  updated_by      TEXT        NOT NULL DEFAULT '',
  updated_at      TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE INDEX IF NOT EXISTS idx_prod_roles_team_lead ON prod_roles(team_lead);

ALTER TABLE prod_roles ENABLE ROW LEVEL SECURITY;

CREATE POLICY "admin_tl_tester can read prod_roles"
  ON prod_roles FOR SELECT
  USING (get_my_role() IN ('admin', 'tl', 'tester'));

CREATE POLICY "admin_tl can insert prod_roles"
  ON prod_roles FOR INSERT
  WITH CHECK (get_my_role() IN ('admin', 'tl'));

CREATE POLICY "admin_tl can update prod_roles"
  ON prod_roles FOR UPDATE
  USING (get_my_role() IN ('admin', 'tl'));

CREATE POLICY "admin can delete prod_roles"
  ON prod_roles FOR DELETE
  USING (get_my_role() = 'admin');

-- Migration (run once in Supabase SQL Editor if table doesn't exist yet):
-- CREATE TABLE IF NOT EXISTS prod_roles ( id UUID PRIMARY KEY DEFAULT uuid_generate_v4(), month TEXT NOT NULL DEFAULT '', team_lead TEXT NOT NULL DEFAULT '', module TEXT NOT NULL DEFAULT '', resource_eid TEXT NOT NULL DEFAULT '', role_details TEXT NOT NULL DEFAULT '', updated_by TEXT NOT NULL DEFAULT '', updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW() );
-- ALTER TABLE prod_roles ENABLE ROW LEVEL SECURITY;
-- If table already exists, add month column: ALTER TABLE prod_roles ADD COLUMN IF NOT EXISTS month TEXT NOT NULL DEFAULT '';
